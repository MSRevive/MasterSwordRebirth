...
string GetExceptionInfo(asIScriptContext *ctx, bool showStack)
{
	...
	text << "{gold}function{white}: {orange}" << function->GetDeclaration() << "\n";
	text << "{gold}Plugin{white}:   {ancient}" << (function->GetModuleName() ? function->GetModuleName() : "") << "\n";
	text << "{gold}Section{white}:  {green}" << (function->GetScriptSectionName() ? function->GetScriptSectionName() : "") << "\n";
	text << "{gold}Line{white}:     {limegreen}" << ctx->GetExceptionLineNumber() << "\n";
	text << "{gold}Error{white}:    {chartreuse}" << ctx->GetExceptionString() << "\n";

	if( showStack )
	{
		text << "{gold}--- {community}CALL STACK {gold}---{white}\n";
...
/**JSON
 * Enumeration of throw methods.
 *
 * @type enum
 * @name ThrowMethod
 *
 * @args m_kDefault # Prints and logs only the input throw message
 * @args m_kDump # Prints and logs a dump without a call stack
 * @args m_kFullDump # Prints and logs a dump with a call stack
 *
 */
static void RegisterThrowMethod( asIScriptEngine &engine )
{
	const char *pszObjectName = "ThrowMethod";

	engine.RegisterEnum( pszObjectName );

	engine.RegisterEnumValue( pszObjectName, "m_kDefault", m_kDefault );
	engine.RegisterEnumValue( pszObjectName, "m_kDump", m_kDump );
	engine.RegisterEnumValue( pszObjectName, "m_kFullDump", m_kFullDump );
}

void ScriptThrow(const string &msg, ThrowMethod_t method)
{
	...
	// Log the exception, then unload it
	const asIScriptFunction *function = ctx->GetExceptionFunction();
	if ( !function->GetModuleName() ) return;

	g_ASManager.ASLogPrintForce( "Exception Handling", UTIL_LogFolder( LOG_ANGELSCRIPT, false ), as::LogType::LOG_EXCEPTION, ( method > m_kDefault ) ? GetExceptionInfo( ctx, ( method > m_kDump ) ? true : false ).c_str() : msg.c_str() );
	...
	for ( size_t uiIndex = 0; uiIndex < g_ASManager.GetPluginManager().GetPluginCount(); ++uiIndex )
	{
		...
		if ( FStrEq( pData->GetName(), modulename.c_str() ) )
		{
			g_ASManager.GetPluginManager().UnloadOnException( pModule );
			break;
		}
	}
#endif
...
/**JSON
 * Throws an exception which will unload the plugin instantly
 *
 * @global true
 * @name throw
 * @type void
 *
 * @args %string_in% message # Our message
 * @args %enum_ThrowMethod% method = m_kDefault # Determines how we should print and log the error
 *
 *
*/
	r = engine->RegisterGlobalFunction("void throw(const string &in msg, ThrowMethod method = m_kDefault)", asFUNCTION(ScriptThrow), asCALL_CDECL); assert(r >= 0);
