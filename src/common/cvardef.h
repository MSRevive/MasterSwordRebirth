...
enum {
	...
	FCVAR_NOEXTRAWHITEPACE = (1 << 9), // strip trailing/leading white space from this cvar
	FCVAR_PRIVILEGED = (1 << 10),	// only available in privileged mode
	FCVAR_FILTERABLE = (1 << 11),	// filtered in unprivileged mode if cl_filterstuffcmd is 1
	FCVAR_ISEXECUTED = (1 << 12),// This cvar's string contains a value that will be executed as a cfg file; don't allow commands to be appended to it
	FCVAR_ISPATH = (1 << 13) // This cvar's string is a path or filename; don't allow absolute paths, escaping to another directory or backslashes
};
