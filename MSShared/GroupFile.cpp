...
/*-----------
	CEGroupFile - File packer with encryption
------------*/
void CEGroupFile::Open(char *pszFileName)
{
...
bool CEGroupFile::WriteEntry(const char *pszName, byte *pData, unsigned long DataSize)
{
...
bool CEGroupFile::ReadEntry(const char *pszName, byte *pBuffer, unsigned long &DataSize)
{
	msstring EntryName = pszName;
	ReplaceChar(EntryName, '\\', '/');
...
bool CEGroupFile::DeleteEntry(const char *pszName)
{
	for (int i = 0; i < m_EntryList.size(); i++)
	{
		cachedentry_t &Entry = m_EntryList[i];
